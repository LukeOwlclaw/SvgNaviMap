...
/**
 * Maps a list of complete scans to a room (ID).
 * Data can be read to and read from arff file.
 * 
 * Created by mkaay on 3/9/14.
 * Corrected by Ohrt on 06/24/14.
 */
public class SSIDMap {
    ...
    private final static String DEFAULT_ROOM_PREFIX = "vertex_";
    ...
    public void readFromFile(File arffFile) throws IOException {
    	...
    	Log.i(LOGTAG, "reading " + arffFile.getName() + " with " + lines.size() + " lines.");
    	clearScanResults();
		for (String line : lines) {
			...
			if (line.contains("@")) {
				if (line.contains("@relation")) {
					...
					if(line.substring(line.indexOf('n') + 2).trim().equalsIgnoreCase(DEFAULT_RELATION_NAME))
					{
						Log.i(LOGTAG, "Reading default relation: " + DEFAULT_RELATION_NAME);
					} else 
					{
						Log.w(LOGTAG, "Cannot read " + arffFile + ". Only reading default relation is supported.");
						return;
					}
				}				
			} else {
				...
				int roomId = 0;
				try {
					roomId = Integer.parseInt(roomName.substring(DEFAULT_ROOM_PREFIX.length()));
				} catch(NumberFormatException nfe) {
					Log.e(LOGTAG, "Room/Vertex/Label has unsupported format: " + roomName + ". Fix arff file.");
					return;
				}
				...
				for (String bssidStrengthItem : bssidStrengthMapList) {
					...
					int bssidStrengh = Integer.parseInt(bssidStrengthItem.substring(bssidStrengthItem
							.indexOf(',') + 1));
				...
				//Log.i(LOGTAG, "reading.. scan for " + roomName+ ": " + completeScanResult.toString());
			}
    ...
    public String toString() {
    	int scanCount = 0;
    	for (Entry<Integer, List<CompleteScanResult>> entry : ssidMap.entrySet()) {
    		scanCount += entry.getValue().size();
		}
    	String s = "SSIDMap: " + ssidMap.size() + " rooms with " + scanCount + " scans";
    	return s;
    }
	...
	public void saveToFile(File arffFile) throws IOException {
        ...
        Log.i(LOGTAG, "saving " + arffFile.getName() + " with " + ssidMap.size() + " rooms.");

        for (Map.Entry<Integer, List<CompleteScanResult>> entry : ssidMap.entrySet()) {
        ...
        for (Map.Entry<Integer, List<CompleteScanResult>> entry : ssidMap.entrySet()) {
        	Log.i(LOGTAG, "saving.. adding " + entry.getValue().size() + " scans for room " + entry.getKey());
