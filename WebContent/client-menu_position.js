...
function position_set(svgid, posX, posY) {
	G.log("called position_set(" + svgid + ", " + posX + ", " + posY + ");");
...
function refresh_location() {
	...
	if (currPositionPoint == null) {
		if (typeof (svgapp) != "undefined")
			...
			document.getElementById('location').innerHTML = htmlHeader + 'Please set your position.';
	...
	for ( var i = 0, v = vertexarray[i]; i < vertexarray.length; v = vertexarray[++i]) {
		...
		if (isPositionInPolygon(currPositionPoint.getX(), currPositionPoint.getY(), v.getPolygon())) {
	...
	if (typeof (svgapp) == "undefined")
		document.getElementById('location').innerHTML = htmlHeader + 'unknown location<br><br>Svgid: '
				+ currPositionPoint.getSvgid() + '.';
